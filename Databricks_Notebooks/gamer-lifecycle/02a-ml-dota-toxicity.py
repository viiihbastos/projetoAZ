...
display(spark.table("BZ_dota_train").filter("toxic == 0"))
...
dataPrepDF = spark.table("BZ_dota_train")\
...
# MAGIC %md
# MAGIC ## Step 6: Load classification model from MLflow Model Registry
# MAGIC 
...
# MAGIC %md
# MAGIC ## Step 7: Productionalizing ML Pipelines with Batch or Streaming
# MAGIC 
...
# MAGIC %md
# MAGIC ##### 7.2 Inference on streaming dataframe
...
checkpoint = f"{tmpdir}/_checkpoints/Toxicity_Chat_Pred"
...
comments_pred.writeStream\
  ...
  .table("SLV_dota_toxicity") \
  .awaitTermination() # set awaitTermination to block subsequent blocks from execution

# COMMAND ----------

# MAGIC %md
# MAGIC ##### 7.4 Display new table with inferred labels

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT match_id, comment_text, slot, time, unit, predicted FROM SLV_dota_toxicity LIMIT 1
...
# MAGIC %md
# MAGIC ## Step 8: Exploring the impact of toxicity on 50K Dota 2 Matches
# MAGIC 
