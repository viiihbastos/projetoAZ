...
def endpoint_exists(serving_endpoint_name):
  ...
  url = f"{serving_host}/api/2.0/serving-endpoints/{serving_endpoint_name}"
...
def wait_for_endpoint(serving_endpoint_name):
  ...
  headers = { 'Authorization': f'Bearer {creds.token}' }
  endpoint_url = f"{serving_host}/api/2.0/serving-endpoints/{serving_endpoint_name}"
...
def create_endpoint(serving_endpoint_name, served_models):
  ...
  endpoint_url = f'{serving_host}/api/2.0/serving-endpoints'
  headers = { 'Authorization': f'Bearer {creds.token}' }
...
def update_endpoint(serving_endpoint_name, served_models):
  ...
  endpoint_url = f"{serving_host}/api/2.0/serving-endpoints/{serving_endpoint_name}/config"
  headers = { 'Authorization': f'Bearer {creds.token}' }
